...
<style>
  ...
  :root{--cis-muted:rgba(127,127,127,.82);--cis-faint:rgba(127,127,127,.62);--cis-line:rgba(127,127,127,.20);--cis-soft:rgba(127,127,127,.09);--cis-ok:#2f9e58;--cis-info:#3d7fba;--cis-warn:#b97916;--cis-error:#c8453c;--cis-active:#2f9e58;}
  *{box-sizing:border-box;margin:0;padding:0;} button{font:inherit;}
  body{background:transparent;color:var(--content-color);font-family:'Poppins',-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;margin:0;padding:{$bodyTopPadding} 8px 8px;font-size:13px;line-height:1.35;font-variant-numeric:tabular-nums;}
  .cis-app{width:100%;}.cis-head{display:flex;align-items:center;justify-content:space-between;gap:12px;padding:2px 4px 10px;border-bottom:1px solid var(--cis-line);margin-bottom:10px;}.cis-head-main{display:flex;align-items:center;gap:9px;min-width:0;}.cis-head-icon{display:grid;place-items:center;width:30px;height:30px;border-radius:9px;background:var(--cis-soft);color:var(--accent-color);font-size:19px;line-height:1;}.cis-kicker{font-size:.68em;color:var(--cis-muted);text-transform:uppercase;letter-spacing:.09em;line-height:1.1;}.cis-title{font-size:1.05em;font-weight:700;line-height:1.2;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;}.cis-head-meta{display:flex;align-items:center;gap:5px;color:var(--cis-muted);font-size:.72em;white-space:nowrap;}.cis-live-dot{width:7px;height:7px;border-radius:50%;background:var(--cis-ok);box-shadow:0 0 0 3px rgba(47,158,88,.14);}
  .grp{margin-bottom:12px;}.grp+.grp{margin-top:12px;}.grp-hdr{display:flex;align-items:stretch;gap:5px;margin-bottom:7px;border-left:3px solid var(--accent-color);border-radius:8px;background:var(--cis-soft);overflow:hidden;}.grp-toggle{display:flex;align-items:center;gap:7px;flex:1;min-width:0;padding:8px 9px;border:0;background:transparent;color:var(--content-color);text-align:left;cursor:pointer;}.grp-toggle:hover{background:rgba(127,127,127,.08);}.grp-toggle:focus-visible,.grp-nav:focus-visible{outline:2px solid var(--accent-color);outline-offset:-2px;}.grp-chevron{flex:0 0 auto;color:var(--accent-color);font-size:1.05em;transition:transform .16s ease;}.grp.is-collapsed .grp-chevron{transform:rotate(-90deg);}.grp-name{font-size:.88em;font-weight:700;flex:0 1 auto;min-width:0;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;}.grp-chips{display:flex;gap:5px;align-items:center;flex:1;min-width:0;flex-wrap:wrap;}.grp-stat{display:inline-flex;align-items:center;gap:3px;padding:2px 6px;border-radius:999px;background:var(--card-color);color:var(--cis-muted);font-size:.76em;white-space:nowrap;}.grp-stat .al-r,.grp-alarm{color:var(--cis-error);}.grp-ok{color:var(--cis-ok);font-size:.76em;font-weight:600;white-space:nowrap;}.grp-nav{display:grid;place-items:center;flex:0 0 32px;border:0;border-left:1px solid var(--cis-line);background:transparent;color:var(--cis-muted);cursor:pointer;font-size:1.1em;}.grp-nav:hover{background:var(--cis-soft);color:var(--accent-color);}.grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(min(100%,185px),1fr));gap:8px;}.grp.is-collapsed .grid{display:none;}
  .card{background:var(--card-color);border:1px solid var(--cis-line);border-left:3px solid var(--cis-line);border-radius:10px;padding:10px 11px;min-width:0;max-width:none;box-shadow:0 1px 2px rgba(0,0,0,.04);transition:border-color .15s ease,transform .15s ease,box-shadow .15s ease;}.card.clickable{position:relative;z-index:1;cursor:pointer;}.card.clickable *{pointer-events:none;}.card.clickable:hover{border-color:var(--accent-color);box-shadow:0 2px 7px rgba(0,0,0,.10);transform:translateY(-1px);}.card.clickable:focus-visible{outline:2px solid var(--accent-color);outline-offset:2px;}.s-alert{border-left-color:var(--cis-error);}.s-warn{border-left-color:var(--cis-warn);}.s-charging{border-left-color:var(--cis-info);}.s-active{border-left-color:var(--cis-active);}.s-dehumid{border-left-color:#2b9eaa;}.s-inactive{border-left-color:var(--cis-line);opacity:.82;}.c-head{display:flex;justify-content:space-between;align-items:baseline;gap:8px;margin-bottom:7px;}.c-name{font-weight:700;color:var(--content-color);font-size:.94em;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;}.c-temp{font-weight:700;font-size:.94em;white-space:nowrap;flex-shrink:0;}.p-row{display:flex;gap:8px;margin-top:4px;min-height:1.45em;}.p-cell{display:flex;align-items:center;gap:4px;flex:1;min-width:0;white-space:nowrap;overflow:hidden;color:var(--cis-muted);font-size:.82em;}.p-ico{font-size:.92em;flex-shrink:0;width:1.2em;text-align:center;}.p-none{color:var(--cis-faint);font-size:.82em;}.ico-muted{color:var(--cis-muted);}.ico-on{color:#cf8a1f;}.ico-warn{color:var(--cis-warn);}.ico-alert{color:var(--cis-error);}.ico-charging{color:var(--cis-info);}.ico-solar{color:#cf8a1f;}.ico-grid-in{color:var(--cis-ok);}.ico-grid-out{color:var(--cis-error);}.ico-active{color:var(--cis-active);}.al-r{color:var(--cis-error);}.al-y{color:var(--cis-warn);}.al-g{color:var(--cis-ok);}.co2dot{display:inline-block;width:7px;height:7px;border-radius:50%;vertical-align:middle;margin-left:2px;flex-shrink:0;}.dot-g{background:var(--cis-ok);}.dot-y{background:var(--cis-warn);}.dot-r{background:var(--cis-error);}.soc-track{background:var(--cis-soft);border-radius:999px;height:5px;margin-top:6px;overflow:hidden;}.soc-fill{height:5px;border-radius:999px;transition:width .3s;}.soc-ok{background:var(--cis-ok);}.soc-low{background:var(--cis-warn);}.soc-crit{background:var(--cis-error);}.trend-up{color:var(--cis-warn);font-size:.72em;vertical-align:middle;}.trend-dn{color:var(--cis-info);font-size:.72em;vertical-align:middle;}.trend-st{color:var(--cis-muted);font-size:.72em;vertical-align:middle;}
  .out-bar{display:flex;align-items:center;flex-wrap:wrap;gap:0;padding:9px 11px;border-radius:10px;margin-bottom:10px;background:var(--card-color);border:1px solid var(--cis-line);border-left:3px solid var(--accent-color);box-shadow:0 1px 2px rgba(0,0,0,.04);}.out-theme-freeze{border-left-color:#5b9bd5;}.out-theme-cold{border-left-color:#82bed4;}.out-theme-cool{border-left-color:#64a978;}.out-theme-mild{border-left-color:var(--cis-ok);}.out-theme-warm{border-left-color:#d58b20;}.out-theme-hot{border-left-color:var(--cis-error);}.out-icon{font-size:1.35em;flex-shrink:0;line-height:1;margin-right:9px;}.out-main{display:flex;align-items:baseline;gap:6px;flex-shrink:0;padding-right:12px;margin-right:4px;border-right:1px solid var(--cis-line);}.out-label{font-size:.68em;font-weight:700;color:var(--cis-muted);text-transform:uppercase;letter-spacing:.07em;}.out-temp{font-size:1.18em;font-weight:700;color:var(--content-color);line-height:1;}.out-cold{color:#5b9bd5;}.out-cool{color:#4a90b8;}.out-warm{color:#c97000;}.out-hot{color:var(--cis-error);}.out-seg{flex:1;min-width:0;display:flex;align-items:center;justify-content:center;padding:0 7px;font-size:.80em;color:var(--cis-muted);border-right:1px solid var(--cis-line);white-space:nowrap;}.out-seg:last-child{border-right:none;}.out-comfort{font-size:.96em;}.out-range{display:flex;gap:6px;}.out-lo{color:#4a90b8;font-weight:700;}.out-hi{color:var(--cis-error);font-weight:700;}.out-bar.clickable{position:relative;z-index:1;cursor:pointer;}.out-bar.clickable *{pointer-events:none;}.out-bar.clickable:hover{border-color:var(--accent-color);}.out-warn{display:inline-flex;align-items:center;gap:3px;padding:2px 7px;border-radius:999px;font-size:.74em;font-weight:700;}.out-warn-0{background:rgba(47,158,88,.14);color:var(--cis-ok);}.out-warn-1{background:rgba(185,121,22,.16);color:var(--cis-warn);}.out-warn-2,.out-warn-3,.out-warn-4{background:rgba(200,69,60,.14);color:var(--cis-error);}.out-warn-10,.out-warn-11{background:rgba(123,31,162,.14);color:#7b1fa2;}.out-warn-seg{flex-direction:column;align-items:flex-start;gap:3px;}.out-uv{display:inline-flex;align-items:center;gap:3px;padding:2px 7px;border-radius:999px;font-size:.74em;font-weight:700;background:var(--cis-soft);color:var(--cis-muted);}.uv-low{color:var(--cis-ok);}.uv-mid,.uv-high{color:var(--cis-warn);}.uv-veryhigh{color:var(--cis-error);}.uv-extreme{color:#7b1fa2;}.out-row2{display:flex;flex-wrap:wrap;width:100%;gap:4px;margin-top:7px;padding-top:7px;border-top:1px solid var(--cis-line);}.out-row2 .out-seg{flex:0 1 auto;padding:0 5px;font-size:.78em;border-right:0;}
...
HTML;
                    ...
                    'items'   => [
                        [
                            ...
                            'columns'  => [
                                ...
                                ['caption' => 'Ziel Navigation', 'name' => 'LinkID', 'width' => '125px', 'add' => 1, 'edit' => ['type' => 'SelectObject']],
                                ['caption' => 'Ziel Variable', 'name' => 'ActionID', 'width' => '125px', 'add' => 1, 'edit' => ['type' => 'SelectVariable']],
                                ['caption' => 'Wert', 'name' => 'ActionValue', 'width' => '85px', 'add' => '', 'edit' => ['type' => 'ValidationTextBox']],
                                ['caption' => 'Ziel Skript', 'name' => 'ScriptID', 'width' => '125px', 'add' => 1, 'edit' => ['type' => 'SelectObject']],
    ...
    private function WrapCard(string $html, string $stateClass, int $linkID): string
    {
        ...
        $replacement = "<div class='card{$stateClass} clickable' role='button' tabindex='0' data-cis-open-object='{$linkID}' onclick='event.preventDefault();event.stopPropagation();openObject({$linkID})' onkeydown='if(event.key===&quot;Enter&quot;||event.key===&quot; &quot;){event.preventDefault();openObject({$linkID})}'>";
        $count = 0;
    ...
    private function BuildQuickActions(): string
    {
        ...
        foreach ($actions as $index => $action) {
            ...
            if ($type === 'navigation') {
                ...
                $html .= "<button type='button' class='quick-action' title='{$label}' aria-label='{$label}' onclick='event.stopPropagation();openObject({$linkID})'>&#9889; {$label}</button>";
                continue;
            }
            if ($type === 'variable') {
    ...
    private function BuildOutdoorBar(): string
    {
        ...
        $clickAttr = $hasLink ? " data-cis-open-object='{$linkID}' onclick='event.preventDefault();event.stopPropagation();openObject({$linkID})'" : '';
    ...
    private function BuildGlobalStatus(array $raeume): string
    {
        ...
        if ($lichterAn === 0 && $fensterOffen === 0 && $tempWarn === 0 && $luftWarn === 0) {
            return "<div class='stat-bar stat-normal'><div class='stat-label'><span class='stat-title'>Hausstatus</span><span class='stat-ok'><i class='fa-solid fa-check'></i> Alles in Ordnung</span></div></div>";
        }
        ...
        return "<div class='stat-bar stat-attention'><div class='stat-label'><span class='stat-title'>Hausstatus</span><span class='stat-subtitle'>Aktuelle Hinweise</span></div><div class='stat-items'>" . implode('', $items) . "</div></div>";
    }
    ...
    private function BuildCard_Raum(array $raum): string
    {
        ...
        $hasLink  = $linkID > 0 && IPS_ObjectExists($linkID);
        $cardAttr = $hasLink
            ? "class='card{$stateClass} clickable' role='button' tabindex='0' data-cis-open-object='{$linkID}' onclick='event.preventDefault();event.stopPropagation();openObject({$linkID})' onkeydown='if(event.key===&quot;Enter&quot;||event.key===&quot; &quot;){event.preventDefault();openObject({$linkID})}'"
